# Writing a Fateline Module

A **module** is a content pack — pure data, no code — that the app loads and
the engine interprets. This is what keeps modules safe and lets them run on
iOS, Android, and web (README §1, §5). The base game itself is a module
(`modules/core`), so anything it does, your module can do too.
...
```yaml
id: com.yourname.my-module # reverse-DNS, globally unique
name: My Module
...
- **Flag** — an ad-hoc variable you just need to remember. Set it from an
  ...
  module id (`com.yourname.my-module/has_pet`) to avoid clashes.
- **Declared stat** — a first-class value that should appear in the UI, be
  ...
  modules. Declare it in `content/definitions/stats.yaml`:
...
Without `exposeAs`, your stat is namespaced (`com.yourname.my-module.karma`)
and private to your module. With it, other modules can read/write `karma`.
...
```
fateline-validate path/to/my-module --with modules/core
```
...
`--with modules/core` so your expansion is tested alongside the base game.

## Share it

1. Push your module to a public GitHub repo (`module.yaml` at the root).
...
[`docs/module-schema.json`](./module-schema.json) (generated from the engine's
own schema).
