...
import { assembleModule } from './assemble.js';
import { unzipModule, commonPrefix } from './unzip.js';
import {
  fromPaste,
  fromUpload,
  fromGithub,
...
import { installModule, loadInstalled } from './install.js';
import { createMemoryModuleStore } from './memoryStore.js';
...
function makeRepoZip(): Uint8Array {
  return zipSync({
    'repo-main/module.yaml': strToU8(MODULE_YAML),
...
describe('assembleModule', () => {
  ...
  it('rejects files with no manifest', () => {
    const result = assembleModule({ 'content/events/a.yaml': '[]' });
...
describe('unzip', () => {
  it('strips the common top-level folder', () => {
    const files = unzipModule(makeRepoZip());
    expect(Object.keys(files).sort()).toEqual(['content/definitions/stats.yaml', 'module.yaml']);
  });
...
describe('sources', () => {
  it('fromPaste wraps text as module.yaml (manifest-wrapped or bare)', () => {
    expect(fromPaste(MANIFEST)['module.yaml']).toBe(MANIFEST);
    expect(fromPaste(MODULE_YAML)['module.yaml']).toBe(MODULE_YAML);
  });

  it('fromUpload handles zip bytes and text files', () => {
    expect(Object.keys(fromUpload('m.zip', makeRepoZip()))).toContain('module.yaml');
    expect(fromUpload('m.yaml', MODULE_YAML)['module.yaml']).toBe(MODULE_YAML);
  ...
  it('fromGithub downloads + unzips via the codeload URL', async () => {
    ...
    expect(files['module.yaml']).toBe(MODULE_YAML);
  });
...
describe('install pipeline', () => {
  it('installs a valid module and registers load order', async () => {
    const { store, data } = createMemoryModuleStore();
    const result = await installModule({ 'module.yaml': MODULE_YAML }, store);
    expect(result.ok).toBe(true);
    expect(data.order).toEqual(['com.test.mod']);

    // Re-installing (update) must not duplicate the load-order entry.
    await installModule({ 'module.yaml': MODULE_YAML }, store);
    expect(data.order).toEqual(['com.test.mod']);
  });
