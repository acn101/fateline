...
import { loadModuleFromDir, extractItems } from './loadFromDir.js';
...
describe('loadModuleFromDir', () => {
  it('reads the §5.1 on-disk layout and validates it', async () => {
    const result = await loadModuleFromDir(dir);
  ...
  it('surfaces validation errors from malformed content', async () => {
    ...
    const result = await loadModuleFromDir(bad);
  ...
  it('loads a manifest-only module with no content/ directory', async () => {
    ...
    await writeFile(
      join(minimal, 'module.yaml'),
      ['id: com.test.min', 'name: Min', 'version: 1.0.0', 'engineVersion: ">=0"', 'author: a'].join(
        '\n',
      ),
    );
    const result = await loadModuleFromDir(minimal);
    expect(result.ok).toBe(true);
    if (result.ok) {
