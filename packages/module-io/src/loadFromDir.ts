 ...
 */
export async function loadModuleFromDir(dir: string): Promise<ValidationResult<FatelineModule>> {
  const manifestRaw = await readFile(join(dir, 'module.yaml'), 'utf8');
  ...
  return validateModule({ manifest, content });
}
