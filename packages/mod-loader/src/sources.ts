...
import { unzipMod } from './unzip.js';

/**
 * The four import sources (README §6). Each resolves to a RawModFiles map
...
export function fromPaste(text: string): RawModFiles {
  // Treat the whole paste as mod.yaml; authors paste a self-contained doc
  // with manifest + content keys, which assembleMod/validateMod accept.
  // If it parses as an object with a `manifest`, re-emit it as such.
  const parsed = parseYaml(text);
  if (parsed && typeof parsed === 'object' && 'manifest' in parsed) {
    return { 'mod.yaml': text };
  }
...
export function fromUpload(filename: string, data: Uint8Array | string): RawModFiles {
  if (filename.endsWith('.zip')) {
    if (typeof data === 'string') throw new Error('Zip upload must be binary, not text.');
    return unzipMod(data);
  }
 ...
 */
export async function fromGithub(url: string, fetcher: Fetcher = fetch): Promise<RawModFiles> {
  ...
  return unzipMod(new Uint8Array(await res.arrayBuffer()));
}
