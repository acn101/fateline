// Emit docs/module-schema.json (JSON Schema) from the Zod module schema, so
...
const outFile = join(repoRoot, 'docs', 'module-schema.json');
const tmp = join(here, 'dist', '.gen-json-schema.mjs');

const entry = `
import { z } from 'zod';
import { writeFile, mkdir } from 'node:fs/promises';
import { moduleSchema } from ${JSON.stringify(join(here, 'src', 'module.ts'))};
const schema = z.toJSONSchema(moduleSchema, { target: 'draft-7' });
