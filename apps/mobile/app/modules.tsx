...
import { fromPaste, fromGithub, installModule } from '@fateline/module-loader';
import { moduleStore, refreshSession, useInstalledIds } from '../src/gameSession';

/**
 * Modules screen (README §7): list installed modules and add new ones via the
 * paste and GitHub-link sources. (File upload + registry browse build on the
 * same installModule pipeline; added with native pickers later.)
 */
export default function ModulesScreen() {
  ...
  const installPaste = () =>
    handleInstall(async () => {
      const result = await installModule(fromPaste(paste), moduleStore);
  ...
  const installGithub = () =>
    handleInstall(async () => {
      const result = await installModule(await fromGithub(github), moduleStore);
      if (!result.ok) throw new Error(result.errors.map((e) => e.message).join('; '));
      setGithub('');
    });
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {installed.length === 0 ? (
          <Text style={styles.muted}>No modules installed yet.</Text>
        ) : (
          installed.map((id) => (
        ...
        <TextInput
          ...
          placeholder="Paste a module's YAML here"
          multiline
          value={paste}
          onChangeText={setPaste}
          accessibilityLabel="Module YAML"
        />
